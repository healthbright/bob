...
For the complete list of every settings key and its options, see our [settings reference guide](/blog/settings-reference).

### [Key Settings Categories](#key-settings-categories)

Settings.json covers several categories that each have dedicated guides:

- **Permissions** - Control which tools Claude can use, which files it can read, and which commands it can run. Define `allow`, `deny`, and `ask` rules. See [permission management](/blog/permission-management).
- **Sandbox** - Isolate bash commands from your filesystem and network with OS-level enforcement. Enable with `/sandbox` or the `sandbox.enabled` setting. See our [sandboxing guide](/blog/permission-management).
...
- **Model** - Set a permanent default model via the `model` key instead of passing `--model` every session. See [model selection strategies](/blog/choosing-the-right-claude-model).
- **Status Line** - Display live context usage, git branch, model name, or cost data in a persistent status bar. See our [status line guide](/blog/statusline-guide).
- **Keybindings** - Customize keyboard shortcuts with `/keybindings` to create `~/.claude/keybindings.json`. See our [keybindings guide](/blog/keybindings-guide).
...
For the full list of ~70 environment variables, see the [settings reference](/blog/settings-reference). For terminal-specific setup like line breaks and notifications, see our [terminal setup guide](/blog/terminal-setup-guide).
...
**Multi-Project Setup**: For monorepos or shared team standards, you can load CLAUDE.md files from additional directories using the `--add-dir` flag. See [Loading CLAUDE.md from Additional Directories](/blog/claude-md-mastery#loading-claudemd-from-additional-directories) for details.

## [What's Next](#whats-next)

You now have Claude Code configured for maximum productivity. Next steps:

- **Try your first AI-powered build**: [Create your first project](/blog/first-project) with proper configuration
- **Master advanced features**: Learn [terminal control techniques](/blog/terminal-main-thread)
- **Optimize performance**: Explore [context management strategies](/blog/context-management)
- **Add more power**: Set up [popular MCP servers](/blog/best-addons)
- **Join the community**: Get help in our [troubleshooting guide](/blog/troubleshooting)
...
First Project](/blog/first-project)[Next

Terminal Setup](/blog/terminal-setup-guide)
