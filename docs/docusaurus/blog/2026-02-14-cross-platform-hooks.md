...
Every hook in [`settings.json`](/blog/configuration-basics) follows this universal pattern:
...
Your [`settings.json` permissions](/blog/permission-management) should include equivalents for both platforms:

Commands that don't exist on a platform simply won't be used. Including both costs nothing, and your Claude Code hooks can reference whichever command is available without hitting permission prompts. For more advanced permission automation, see the [Permission Hook guide](/blog/permission-hook-guide).
...
- Configure [Skill Activation](/blog/skill-activation-hook) for automatic skill loading
- Explore [Setup Hooks](/blog/claude-code-setup-hooks) for cross-platform onboarding
- Master [permission rules](/blog/permission-management) alongside cross-platform hook permissions
