...
Check `/tasks` anytime to see background agent progress. Results surface when complete. See [async workflows](/blog/async-workflows) for the full background execution guide.
...
Use `Task(AgentName)` rules in your [permission settings](/blog/permission-management) to control which sub-agents can be invoked:
...
Start with the routing rules. Add them to your [CLAUDE.md](/blog/claude-md-mastery) today and watch your central AI make smarter delegation decisions. For deeper patterns, explore [task distribution](/blog/task-distribution) and [agent fundamentals](/blog/agent-fundamentals).

Last updated on

[Previous

Async Workflows](/blog/async-workflows)[Next

Sub-Agent Design](/blog/sub-agent-design)
