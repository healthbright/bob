...
**Understanding**: Claude Code provides multiple ways to create agent-like behavior - from built-in sub-agents to custom slash commands. Each approach serves different needs. Master these fundamentals before exploring [advanced agent patterns](/blog/agent-patterns).
...
**New**: When Claude spawns sub-agents, you can now background them with `Ctrl+B` and continue chatting with the main agent on other tasks. Results surface automatically when complete. See [async workflows](/blog/async-workflows) for the full guide.

Learn more about [sub-agent design patterns](/blog/sub-agent-design) for complex orchestration.
...
You can control which sub-agents Claude is allowed to invoke using `Task(AgentName)` [permission rules](/blog/permission-management). Add these to the `deny` array in your `settings.json` or use the `--disallowedTools` CLI flag:
...
Your [CLAUDE.md file](/blog/claude-md-mastery) shapes Claude's behavior for every interaction in your project:
...
**Next Action**: Create your first slash command in `.claude/commands/` for a workflow you repeat often, then explore [task distribution strategies](/blog/task-distribution) for complex orchestration.

**Explore More Agent Concepts**:

- [Sub-Agent Best Practices](/blog/sub-agent-best-practices) - When to use parallel, sequential, or background execution
- [Sub-Agent Design](/blog/sub-agent-design) - Architecture patterns for orchestrating multiple agents
- [Custom Agents](/blog/custom-agents) - Build specialized agents with slash commands
- [Agent Patterns](/blog/agent-patterns) - Proven design patterns for agent systems
- [Human-Like Agents](/blog/human-like-agents) - Create agents with distinct personalities

Last updated on

[Previous

Efficiency Patterns](/blog/efficiency-patterns)[Next

Async Workflows](/blog/async-workflows)
