...
For sessions involving large codebases, extensive debugging, or multi-file refactoring, `sonnet[1m]` may be more cost-effective than repeatedly losing context to compaction. See the [model selection guide](/blog/choosing-the-right-claude-model) for the full model alias reference.
...
- Use [session files](/blog/task-management) to persist state
...
- [Memory Optimization](/blog/memory-optimization) - Reduce static context overhead
...
Context Engineering](/blog/context-engineering)[Next

Claude Skills Guide](/blog/claude-skills-guide)
