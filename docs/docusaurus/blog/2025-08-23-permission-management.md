...
You can also [customize this keybinding](/blog/keybindings-guide) if `Shift+Tab` conflicts with your terminal. Now you control your workflow without touching config files. Match your mode to your task and stop the interruption cycle.
...
[PreToolUse hooks](/blog/hooks-guide) run before the permission system and can approve, deny, or modify tool calls at runtime. This gives you programmatic control over permissions beyond static rules. See the [Permission Hook guide](/blog/permission-hook-guide) for a production-ready implementation.
...
These are system-wide paths (not user home directories) requiring administrator privileges. They follow the same format as regular settings files but take the highest precedence in the [settings hierarchy](/blog/configuration-basics).
...
- Automate permission decisions with [hooks](/blog/hooks-guide) and the [Permission Hook](/blog/permission-hook-guide)
- Customize your [keybindings](/blog/keybindings-guide) for faster mode cycling
- Optimize your [feedback loops](/blog/feedback-loops) for faster iteration
- Set up efficient [todo workflows](/blog/todo-workflows) for task management
- Configure [git integration](/blog/git-integration) for seamless version control
- Explore [configuration basics](/blog/configuration-basics) for settings.json and advanced setup
...
Git Integration](/blog/git-integration)[Next
