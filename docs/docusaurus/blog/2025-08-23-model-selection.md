...
**Problem**: Developers waste money using Opus for everything, or struggle with Haiku when they need more power. Tactical model switching can [optimize your usage costs](/blog/usage-optimization) by 60-80%.
...
The `sonnet[1m]` alias is available to Console and API users and unlocks a 1 million token context window. This is valuable for extended sessions where you'd otherwise hit [context compaction limits](/blog/context-buffer-management). Note that extended context models have different pricing.
...
`opusplan` is an excellent choice for refactoring sessions, feature planning, and any workflow where you use [planning mode](/blog/planning-modes) regularly. It is also a strong [cost optimization strategy](/blog/usage-optimization) since you only pay Opus rates during the planning phase.
...
4. **Settings file** - The `model` field in [settings.json](/blog/configuration-basics) for permanent configuration (lowest priority)
...
**For learning**: Check our [configuration guide](/blog/configuration-basics) to set up model defaults.
...
**Next Action**: Set up your optimal model configuration with our [performance optimization guide](/blog/speed-optimization), then learn advanced [context management](/blog/context-preservation) techniques.

For detailed specs on every Claude model Anthropic has released, see the [complete model timeline](/blog).

**Related Guides**:

- [Deep Thinking Techniques](/blog/deep-thinking-techniques) - When to use extended thinking with Opus
- [Efficiency Patterns](/blog/efficiency-patterns) - Build frameworks for consistent output
- [Usage Optimization](/blog/usage-optimization) - Track and reduce your costs

Last updated on

[Previous

Claude Models](/blog)[Next

Claude Opus 4.5 Guide](/blog/claude-opus-4-5-guide)
