...
- [Permission rules](/blog/permission-management) everyone should follow
...
These live inside the `"permissions"` object. For a deep dive into how permissions work in practice, see the [permission management guide](/blog/permission-management).
...
For a full walkthrough of MCP configuration, see the [MCP basics guide](/blog/mcp-basics).
...
**[MCP servers](/blog/mcp-basics)** extend Claude with additional tools. Their configurations live in separate files (`.mcp.json` for project, `~/.claude.json` for user), but settings control which MCP servers get approved and which get blocked.
...
For a broader overview of the three configuration surfaces (CLAUDE.md, MCP, settings), start with the [configuration basics guide](/blog/configuration-basics).
...
- Explore [MCP servers](/blog/mcp-basics) for extending Claude's capabilities
- Learn [permission management](/blog/permission-management) patterns for team security
- Review [configuration basics](/blog/configuration-basics) for the full configuration picture

Last updated on

[Previous

Sandboxing Guide](/blog/permission-management)[Next
