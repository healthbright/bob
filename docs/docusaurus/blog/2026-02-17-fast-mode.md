...
You can also set it permanently in your user settings file. See the [settings reference](/blog/settings-reference) for all available configuration options.
...
- **Disabling keeps your model.** When you toggle fast mode off with `/fast`, you stay on Opus 4.6. Use `/model` to [switch to a different model](/blog/speed-optimization) if needed.
...
- **Compatible with 1M extended context.** Fast mode works with the full [1M context window](/blog/context-management), though pricing increases above 200K tokens.
...
You can combine both. Running fast mode with a reduced effort level stacks the 2.5x infrastructure speed with less thinking overhead. This combination works well for straightforward tasks like formatting, simple refactors, or generating boilerplate where [efficiency patterns](/blog/efficiency-patterns) matter more than deep analysis. For complex architectural decisions or tricky debugging, keep the effort level high and let fast mode handle the speed. The two settings give you independent control over quality and latency, which means you can optimize for each task individually rather than settling for a single compromise.

For a deeper look at how effort levels affect output quality, see our guide on [deep thinking techniques](/blog/deep-thinking-techniques).
...
- **Monitor through Console.** Check your [usage and billing](/blog/usage-optimization) in the Anthropic Console dashboard to track how fast mode affects your spend.
...
2. **[Effort levels](/blog/deep-thinking-techniques)** control thinking depth
3. **[Model selection](/blog/speed-optimization)** controls the base capability tier
