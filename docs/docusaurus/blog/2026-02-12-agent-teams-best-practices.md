...
| Too many permission prompts | Pre-approve common operations in your [permission settings](/blog/permission-management) before spawning teammates. Each teammate inherits the lead's permissions, so configuring once covers the whole team. |
...
- **Permissions set at spawn**: All teammates start with the lead's [permission settings](/blog/permission-management). You can change individual modes after spawning, but not at spawn time.
