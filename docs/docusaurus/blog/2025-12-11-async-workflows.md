...
**Instant compaction**: The `/compact` command now executes immediately. Claude maintains a continuous session memory, so compaction just loads the summary into a fresh context. Learn more in our [context management guide](/blog/context-management).

**Session memory architecture**: Every session now maintains a structured summary including status, completed work, discussion points, and a work log. See [memory optimization](/blog/memory-optimization) for details.
...
**MCP quick toggle**: Enable or disable MCP servers without editing config files. See our [MCP basics guide](/blog/mcp-basics) for the commands.
...
- **Execution patterns**: Master [parallel, sequential, and background patterns](/blog/sub-agent-best-practices) to choose the right approach
- **Foundation**: Understand [agent fundamentals](/blog/agent-fundamentals) before building complex workflows
- **Design patterns**: Learn [sub-agent design](/blog/sub-agent-design) for effective task delegation
- **Distribution**: Master [task distribution](/blog/task-distribution) for multi-agent orchestration
- **Custom agents**: Build specialized agents with [custom agent patterns](/blog/custom-agents)
...
Agent Fundamentals](/blog/agent-fundamentals)[Next

Sub-Agent Patterns](/blog/sub-agent-best-practices)
