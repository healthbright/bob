...
Tasks are scoped per session via `CLAUDE_CODE_TASK_LIST_ID`. Bob Memory is shared across sessions — task references from memory that don't appear in your `TaskList` belong to another session. **`TaskList` is the sole source of truth.**
...
**`/spec` reviewer agents** (`bob:plan-reviewer`, `bob:spec-reviewer`) pass through silently — these are expected parts of the workflow.
...
|--------|---------|---------|---------------|
| Worktree Support | `$BOB_WORKTREE_ENABLED` | `false` | `/spec` never asks about worktree; always passes `--worktree=no` |
| Ask Questions During Planning | `$BOB_PLAN_QUESTIONS_ENABLED` | `true` | `spec-plan` and `spec-bugfix-plan` skip all `AskUserQuestion` calls; make autonomous default choices |
| Plan Approval | `$BOB_PLAN_APPROVAL_ENABLED` | `true` | Plan is auto-approved (`Approved: Yes`); implementation starts immediately |

These env vars are set by the Bob wrapper from `~/.bob/config.json → specWorkflow`. When unset (non-Bob invocations), all three default to enabled.
...
```bash
~/.bob/bin/bob register-plan "<plan_path>" "<status>" 2>/dev/null || true
```
...
1. **Worktree Choice + Type Confirmation** (new plans only, in dispatcher — type only asked when ambiguous; skipped when `$BOB_WORKTREE_ENABLED=false`)
2. **Plan Approval** (in spec-plan or spec-bugfix-plan; skipped when `$BOB_PLAN_APPROVAL_ENABLED=false`)
...
**Zero-interaction mode:** When `$BOB_WORKTREE_ENABLED=false`, `$BOB_PLAN_QUESTIONS_ENABLED=false`, and `$BOB_PLAN_APPROVAL_ENABLED=false`, `/spec` runs completely autonomously from invocation to verified completion. Configure via Console Settings → Spec Workflow.
