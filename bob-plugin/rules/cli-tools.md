...
The `bob` binary is at `~/.bob/bin/bob`. Do NOT call commands not listed here.

**Session & Context:**

| Command                               | Purpose                                  |
| ------------------------------------- | ---------------------------------------- |
| `bob check-context --json`          | Get context usage % (informational only) |
| `bob register-plan <path> <status>` | Associate plan with session              |

**Worktree:** `bob worktree detect|create|diff|sync|cleanup|status --json <slug>`

Slug = plan filename without date prefix and `.md`. `create` auto-stashes uncommitted changes.

**License:** `bob activate <key>`, `bob deactivate`, `bob status`, `bob verify`, `bob trial --check|--start`

**Other:** `bob greet`, `bob statusline`

**Do NOT exist:** ~~`bob pipe`~~, ~~`bob init`~~, ~~`bob update`~~
