...
**Use `-s=$BOB_SESSION_ID` on ALL `playwright-cli` commands:**
...
**Why:** Each `/spec` session gets a unique `BOB_SESSION_ID`. The `-s=` flag gives each session its own isolated browser instance, preventing parallel workflows from interfering with each other.
...
**In `/spec` workflows**, the session name is always `$BOB_SESSION_ID` — never invent custom names. This ensures each parallel spec run is fully isolated.
