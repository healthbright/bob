"""PreCompact hook - capture Bob state before compaction.

Fires before Claude Code compaction to preserve Bob-specific session state
(active plan, task list, context) to Bob Memory for post-compaction restoration.
"""
...
def _capture_task_list() -> dict | None:
    ...
    try:
        pid = os.environ.get("BOB_SESSION_ID", "")
        if not pid:
            return None
        claude_config = Path(os.environ.get("CLAUDE_CONFIG_DIR", str(Path.home() / ".claude")))
        tasks_dir = claude_config / "tasks" / f"bob-{pid}"
...
def run_pre_compact() -> int:
    ...
    session_id = hook_data.get("session_id", os.environ.get("BOB_SESSION_ID", "default"))
    ...
    if saved_to_api:
        print("🔄 Compaction in progress — Bob state captured to memory", file=sys.stderr)
    else:
        print("🔄 Compaction in progress — Bob state captured to local file (worker unavailable)", file=sys.stderr)
