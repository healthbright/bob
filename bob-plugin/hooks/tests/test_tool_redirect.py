...
class TestAllowedSpecReviewerAgents:
    ...
    def test_allows_plan_reviewer(self):
        code, output = _run_with_input("Agent", {"subagent_type": "bob:plan-reviewer", "prompt": "review plan"})
        assert code == 0
        assert output == ""

    def test_allows_spec_reviewer(self):
        code, output = _run_with_input("Agent", {"subagent_type": "bob:spec-reviewer", "prompt": "review code"})
        assert code == 0
        assert output == ""
...
class TestSubprocessIntegration:
    ...
    def test_spec_reviewers_silent(self):
        for subagent in ["bob:spec-reviewer", "bob:plan-reviewer"]:
