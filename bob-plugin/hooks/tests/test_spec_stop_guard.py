...
all notifications via `bob notify`. The stop guard only blocks/allows stops.
"""
...
def _test_session_dir() -> Path:
    ...
    return Path.home() / ".bob" / "sessions" / TEST_SESSION_ID
...
def _run_subprocess(input_data: dict, plans_dir: Path | None = None) -> tuple[int, str, str]:
    ...
    env = {**os.environ, "BOB_SESSION_ID": TEST_SESSION_ID}
...
class TestGetStopGuardPath:
    ...
    def test_creates_parent_directory(self, tmp_path: Path) -> None:
        ...
        with (
            patch.dict(os.environ, {"BOB_SESSION_ID": "777"}),
