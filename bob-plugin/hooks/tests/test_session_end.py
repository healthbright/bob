...
def test_skips_stop_when_other_sessions_active(tmp_path: Path):
    """Should skip worker stop when other Bob sessions are running."""
    base = tmp_path / "sessions"
    (base / "1001").mkdir(parents=True)
    (base / "2002").mkdir(parents=True)

    with (
        patch.dict(os.environ, {"CLAUDE_PLUGIN_ROOT": "/fake/plugin", "BOB_SESSION_ID": "1001"}),
...
def test_main_calls_complete_session_before_worker_stop(tmp_path: Path):
    ...
    with (
        patch.dict(
            os.environ,
            {"CLAUDE_PLUGIN_ROOT": "/fake/plugin", "BOB_SESSION_ID": "1001", "CLAUDE_SESSION_ID": "test-id"},
        ),
        patch.object(session_end, "SESSIONS_DIR", base),
