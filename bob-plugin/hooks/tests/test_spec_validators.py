    ...
    def test_allows_stop_when_plan_created(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            result = subprocess.run(
                [sys.executable, "bob-plugin/hooks/spec_plan_validator.py"],
...
class TestSpecVerifyValidator:
    ...
    def _setup_active_plan(self, plan_path: Path) -> Path:
        ...
        session_dir = Path.home() / ".bob" / "sessions" / self.TEST_SESSION_ID
    ...
    def _run_validator(self, input_data: dict) -> subprocess.CompletedProcess:
        """Run spec_verify_validator with isolated BOB_SESSION_ID."""
        env = {**os.environ, "BOB_SESSION_ID": self.TEST_SESSION_ID}
        return subprocess.run(
            [sys.executable, "bob-plugin/hooks/spec_verify_validator.py"],
