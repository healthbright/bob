...
def _get_max_context_tokens() -> int:
    ...
    """
    try:
        session_id = os.environ.get("BOB_SESSION_ID", "").strip()
        if session_id:
            cache_file = Path.home() / ".bob" / "sessions" / session_id / "context-pct.json"
...
def get_session_cache_path() -> Path:
    ...
    session_id = os.environ.get("BOB_SESSION_ID", "").strip() or "default"
