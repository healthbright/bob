...
name: mcp-servers
description: |
  Bob MCP server reference with API details, parameters, and usage examples.
...
All Bob MCP servers use the `mcp__plugin_bob_` prefix. Tools are available immediately after ToolSearch returns them.
...
# Then call directly
mcp__plugin_bob_mem-search__search(query="authentication flow", limit=5)
mcp__plugin_bob_mem-search__timeline(anchor=22865, depth_before=3, depth_after=3)
mcp__plugin_bob_mem-search__get_observations(ids=[22865, 22866])
mcp__plugin_bob_mem-search__save_memory(text="Important finding", title="Short title")
```
...
mcp__plugin_bob_context7__resolve-library-id(query="how to use fixtures", libraryName="pytest")
# → returns libraryId like "/pypi/pytest"
mcp__plugin_bob_context7__query-docs(libraryId="/pypi/pytest", query="how to create and use fixtures")
```
...
mcp__plugin_bob_web-search__search(query="Python asyncio best practices 2026", limit=5)
mcp__plugin_bob_web-search__fetchGithubReadme(url="https://github.com/astral-sh/ruff")
```
...
mcp__plugin_bob_grep-mcp__searchGitHub(query="FastMCP", language=["Python"])
mcp__plugin_bob_grep-mcp__searchGitHub(query="(?s)useEffect\\(.*cleanup", useRegexp=True, language=["TypeScript"])
```
...
mcp__plugin_bob_web-fetch__fetch_url(url="https://docs.example.com/api")
mcp__plugin_bob_web-fetch__fetch_urls(urls=["https://a.com", "https://b.com"])
```
...
mcp__plugin_bob_codebase-memory-mcp__search_graph(label="Function", name_pattern=".*Handler.*")
mcp__plugin_bob_codebase-memory-mcp__trace_call_path(function_name="ProcessOrder", direction="both", depth=2)
mcp__plugin_bob_codebase-memory-mcp__detect_changes(scope="all")
mcp__plugin_bob_codebase-memory-mcp__get_code_snippet(qualified_name="MyClass", auto_resolve=true, include_neighbors=true)
```
