...
**Use the Share page in the Bob Console dashboard** (`http://localhost:41777/#/share`):
...
**Note:** Bob-managed rules/commands (installed by the Bob installer) are tracked via manifest and should NOT be placed in extras. Extras are for user-created assets you want to share across machines.
...
Skills are shared with users who may not have Bob. **Only reference built-in Claude Code tools** in skill content:

| Use This (Built-in) | NOT This (Bob-specific) |
|---------------------|--------------------------|
| `Grep`, `Glob` | `probe search/extract/query` |
| `Bash` + `npx playwright` | `playwright-cli` |
| `WebFetch`, `WebSearch` | Bob MCP servers |
| `Bash` + standard CLI | `bob` CLI, `skillshare` CLI |
