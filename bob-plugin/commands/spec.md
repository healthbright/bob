...
```bash
echo "WORKTREE=$BOB_WORKTREE_ENABLED QUESTIONS=$BOB_PLAN_QUESTIONS_ENABLED APPROVAL=$BOB_PLAN_APPROVAL_ENABLED"
```
...
Read plan, register association: `~/.bob/bin/bob register-plan "<plan_path>" "<status>" 2>/dev/null || true`
