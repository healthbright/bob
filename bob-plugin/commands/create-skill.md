...
# Result: "bob", "my-api", "acme-backend"
```
...
**Naming rules:** Lowercase with hyphens only. The slug provides context; the name should be 1-3 words max that are descriptive (not generic). Examples: `bob-lsp-cleaner`, `my-api-auth-flow`, `acme-deploy`. Never use generic names like "helper", "utils", "tools", "handler", "workflow".
...
# Global skills (source + Bob defaults)
ls ~/.config/skillshare/skills/ 2>/dev/null
ls ~/.claude/bob/skills/ 2>/dev/null
rg -i "keyword" ~/.config/skillshare/skills/ ~/.claude/bob/skills/ 2>/dev/null
...
**Portability checklist** — skills are shared with users who may NOT have Bob:
...
- **Never reference Bob-specific tools:** `probe search/extract/query`, `playwright-cli`, `skillshare`, `bob` CLI, Bob MCP servers (`mem-search`, `context7`, `grep-mcp`, `web-fetch`, `web-search`)
