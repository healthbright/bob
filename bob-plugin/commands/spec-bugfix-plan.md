...
- **⛔ If `BOB_PLAN_QUESTIONS_ENABLED` is `"false"` (from Step 0),** skip all `AskUserQuestion` calls (Steps 1.2.1, 1.2.5 escalation). Make reasonable default assumptions and document them in the plan. Continue autonomously.
...
```bash
echo "QUESTIONS=$BOB_PLAN_QUESTIONS_ENABLED APPROVAL=$BOB_PLAN_APPROVAL_ENABLED"
```
...
5. **Register:** `~/.bob/bin/bob register-plan "<plan_path>" "PENDING" 2>/dev/null || true`
...
**When `BOB_PLAN_APPROVAL_ENABLED` is NOT `"false"`:**

0. Notify:
   ```bash
   ~/.bob/bin/bob notify plan_approval "Bugfix Plan Ready" "<plan-slug> — approval needed" --plan-path "<plan_path>" 2>/dev/null || true
   ```
