...
2. Detect: `~/.bob/bin/bob worktree detect --json <plan_slug>`
...
6. **Run actual program** — use plan's Runtime Environment. Check port: `lsof -i :<port>`. If using playwright-cli: `-s="${BOB_SESSION_ID:-default}"`
...
4. Register: `~/.bob/bin/bob register-plan "<plan_path>" "COMPLETE" 2>/dev/null || true`
