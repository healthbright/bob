...
- **Run plan-reviewer when enabled** — it runs for every feature spec when `$BOB_PLAN_REVIEWER_ENABLED` is not `"false"`. Context level is NOT a valid reason to skip. To disable, use Console Settings → Reviewers → Plan Review toggle.
...
```bash
echo "QUESTIONS=$BOB_PLAN_QUESTIONS_ENABLED REVIEWER=$BOB_PLAN_REVIEWER_ENABLED APPROVAL=$BOB_PLAN_APPROVAL_ENABLED"
```
...
**⛔ If `BOB_PLAN_QUESTIONS_ENABLED` is `"false"` (from Step 0),** skip ALL `AskUserQuestion` calls in Steps 1.2, 1.3b, and 1.4. Make reasonable default choices and document them in the plan under an "Autonomous Decisions" sub-section. Continue to the next step immediately.
...
2. **Create worktree early (if yes):**
   ...
   ```bash
   ~/.bob/bin/bob worktree detect --json <plan_slug>
   # If not found:
   ~/.bob/bin/bob worktree create --json <plan_slug>
...
5. **Register plan:** `~/.bob/bin/bob register-plan "<plan_path>" "PENDING" 2>/dev/null || true`
...
3. **Ask Batch 1 questions** → notify, then use `AskUserQuestion` with each question as a separate entry with predefined options:
   ...
   ```bash
   ~/.bob/bin/bob notify plan_approval "Input Needed" "<plan_name> — clarification questions" --plan-path "<plan_path>" 2>/dev/null || true
   ```
...
```bash
~/.bob/bin/bob notify plan_approval "Design Decisions" "<plan_name> — architecture choices" --plan-path "<plan_path>" 2>/dev/null || true
```
...
**⛔ If `BOB_PLAN_REVIEWER_ENABLED` is `"false"` (from Step 0),** skip this step entirely and proceed to Step 1.8.
...
Output path: `~/.bob/sessions/<SESS_ID>/findings-plan-reviewer-<plan-slug>.json`
...
```
Task(
  subagent_type="bob:plan-reviewer",
...
**⛔ If `BOB_PLAN_APPROVAL_ENABLED` is `"false"` (from Step 0),** skip this step: set `Approved: Yes` in the plan file automatically and immediately invoke `Skill(skill='spec-implement', args='<plan-path>')`. No AskUserQuestion, no notification.

**When `BOB_PLAN_APPROVAL_ENABLED` is NOT `"false"` — MANDATORY APPROVAL GATE:**

0. Notify:

   ```bash
   ~/.bob/bin/bob notify plan_approval "Plan Ready for Review" "<plan_name> — approval needed" --plan-path "<plan_path>" 2>/dev/null || true
   ```
