---
description: Set up and audit project rules — reads codebase, generates modular rules, documents MCP servers, creates AGENTS.md for cross-tool compatibility
user-invocable: true
model: opus
---
# /setup-rules - Set Up Project Rules

**Set up and audit project rules.** Reads your codebase, generates modular rules, documents MCP servers, and creates an `AGENTS.md` for cross-tool compatibility.

**Flow:** Read existing → Migrate → Quality audit → Explore → Compare → Sync rules → Sync MCP → Discover rules → Generate AGENTS.md → Cross-check → Summary

**Skill creation:** Use `/create-skill` to create workflow skills — `/setup-rules` focuses exclusively on rules and cross-tool compatibility.
...
- `project.md` → `{slug}-project.md` | `mcp-servers.md` → `{slug}-mcp-servers.md` | `{topic}.md` → `{slug}-{topic}.md` (unless managed by a framework like Bob)
...
**Document user-configured MCP servers.** Skip framework-provided servers (e.g., Bob core: context7, mem-search, web-search, web-fetch, grep-mcp) — only document servers the user added themselves.

### Step 7.1: Discover

Parse `.mcp.json`, exclude framework-provided servers (Bob core servers if present: context7, mem-search, web-search, web-fetch, grep-mcp).
...
## Phase 9: Generate AGENTS.md

**Create or update `AGENTS.md` at the project root** — a universal file that works with any AI coding tool (Claude Code, Cursor, Codex, Amp, Jules, and others). **AGENTS.md is the ONLY file these tools can read** — it must contain all tribal knowledge from `.claude/rules/`, not just a summary of the project rule.

**About AGENTS.md:** The [AGENTS.md standard](https://agents.md/) is used by 60k+ open-source projects. It's plain markdown — no tool-specific syntax, no slash commands. Content must be tool-agnostic.

**Key principle:** `.claude/rules/` files are modular for Claude Code. AGENTS.md consolidates them into one comprehensive file for every other tool.

### Step 9.1: Check Existing

```bash
ls AGENTS.md 2>/dev/null
```

If it exists: read it. Check for the auto-generation header (`Auto-generated by /setup-rules`).

- **Auto-generated:** Regenerate from current rules state.
- **Hand-written or externally generated:** Treat like an existing CLAUDE.md — AskUserQuestion: "Found existing AGENTS.md (not generated by /setup-rules). How to proceed?"
  - **"Merge with rules"** — Read existing content, merge unique sections with rule-derived content, preserve anything not covered by rules
  - **"Regenerate (replaces existing)"** — Full regeneration from rules. Existing content is overwritten (except Custom section).
  - **"Skip AGENTS.md"** — Leave as-is

### Step 9.2: Gather Content

**Read ALL rule files in `.claude/rules/`** — not just the project rule:

1. `{slug}-project.md` — tech stack, directory structure, development commands, architecture
2. **Every other `{slug}-*.md` rule** — git commit standards, installer patterns, coding conventions, testing patterns, deployment conventions, etc.
3. `CLAUDE.md` if it exists
4. `README.md` — for project description and any conventions documented there
5. Nested product/team rules (if they exist)

**For each rule file, extract the tool-agnostic content:**

| Source Rule | What to Extract | What to Skip |
|-------------|-----------------|--------------|
| `{slug}-project.md` | Tech stack, structure, commands, architecture | References to `~/.claude/`, Bob-specific tooling |
| `{slug}-git-commits.md` | Commit format, semantic-release types, message rules | AI-specific instructions ("wait for user to say commit") |
| `{slug}-installer-patterns.md` | Three-way merge logic, manifest tracking, non-destructive rules | — |
| `{slug}-mcp-servers.md` | Server names, purposes, available tools, workflows | `ToolSearch` invocation syntax, Claude-specific calling conventions |
| `{slug}-conventions.md` | Code style, patterns, gotchas | AI tool references |
| Any other `{slug}-*.md` | Domain knowledge, architectural decisions, gotchas | Tool-specific workflows |

**What NOT to include in AGENTS.md:**
- Claude Code slash commands (`/spec`, `/setup-rules`, etc.)
- References to `~/.claude/` directories or Bob-specific tooling
- Claude-specific MCP invocation syntax (`ToolSearch`, `mcp__plugin_bob_*` tool names)
- AI behavior instructions ("ask Claude to...", "wait for user permission")
- Generic development practices that any AI agent already knows
- File-by-file descriptions that can be easily discovered by reading code

### Step 9.3: Write AGENTS.md

**Target 150-300 lines.** Every line must earn its place. Write rules like clear internal docs — focused, actionable, scoped.

```markdown
<!--
  AGENTS.md — AI coding agent context for [Project Name]
  Auto-generated by /setup-rules on [Date]. Edit source rules in .claude/rules/ and re-run /setup-rules to regenerate.
...
[Language, framework, build tool, package manager — include versions where they matter]
...
| Build | `[command]` |
| Test | `[command]` |
| Single test | `[command for running one test]` |
| Lint | `[command]` |

## Directory Structure

[Key directories with one-line descriptions — focus on what requires reading multiple files to understand]

## Architecture

[High-level architecture that isn't obvious from the directory structure. How components connect, data flow, key patterns. This is the "big picture" that requires reading multiple files to understand.]

## [Topic from {slug}-conventions.md or similar rule]

[Consolidated tool-agnostic content from each non-project rule file. Each gets its own section.]

## [Topic from {slug}-git-commits.md]

[Commit standards, release automation, message format]

## MCP Servers

[Server names, purposes, key tools, workflows — from {slug}-mcp-servers.md. Skip Claude-specific invocation syntax.]

## Testing

[How to write and run tests, key patterns, coverage requirements]

## Key Gotchas

[Non-obvious behaviors, common mistakes, things that will trip up a new contributor]

<!-- Custom: content below this line is preserved on regeneration -->
```

**Writing quality checklist (apply to every section):**
- Lead with the rule — what to do first, why second
- Code examples over prose — show, don't tell
- Skip the obvious — don't document standard language/framework behavior
- Bullet points > paragraphs — scannable beats readable
- Use IMPORTANT for critical rules (data loss, breaking changes) — sparingly
- Don't repeat yourself across sections
- Commands must be runnable in any terminal (no tool-specific wrappers)

### Step 9.4: Preserve Custom Content

If the existing AGENTS.md has content after `<!-- Custom: ... -->`, preserve it verbatim when regenerating.
...
8. **AGENTS.md accuracy** — verify that commands in AGENTS.md actually exist in the project (`which <cmd>` or `grep -r "<cmd>"`)
...
- AGENTS.md: created | updated | skipped (note reason)
