...
1. **Run code review when enabled** — Step 3.1 launches `spec-reviewer` via `Task(subagent_type="bob:spec-reviewer")` when `BOB_SPEC_REVIEWER_ENABLED` is not `"false"` (read in Step 0). To disable, use Console Settings → Reviewers → Code Review toggle.
...
```bash
echo "REVIEWER=$BOB_SPEC_REVIEWER_ENABLED"
```
...
```bash
rm -f ~/.bob/sessions/$BOB_SESSION_ID/findings-plan-reviewer-*.json
```

### Step 3.1: Launch Code Review Agent (Early)

**⛔ If `BOB_SPEC_REVIEWER_ENABLED` is `"false"` (from Step 0),** skip this step entirely and proceed to Step 3.2. (Automated checks in Step 3.2 still run; only the agent-based review is skipped.)
...
**Validate session ID:** If `$BOB_SESSION_ID` is empty, fall back to `"default"` to avoid writing to `~/.bob/sessions//`.
...
Output path: `~/.bob/sessions/<session-id>/findings-spec-reviewer-<plan-slug>.json`
...
```bash
rm -f ~/.bob/sessions/$BOB_SESSION_ID/findings-spec-reviewer-*.json
```

```
Task(
  subagent_type="bob:spec-reviewer",
...
**⛔ If `BOB_SPEC_REVIEWER_ENABLED` is `"false"` (from Step 0 — Step 3.1 was skipped),** skip this step entirely and proceed to Step 3.6 (Phase B). There are no findings to collect.
...
**⛔ If `BOB_SPEC_REVIEWER_ENABLED` is `"false"` (from Step 0 — Steps 3.1/3.4 were skipped),** skip this step entirely and proceed to Phase B.
...
```bash
PW_SESSION="${BOB_SESSION_ID:-default}"
...
2. Check: `~/.bob/bin/bob worktree detect --json <plan_slug>`
...
6. **Show diff:** `~/.bob/bin/bob worktree diff --json <plan_slug>`

7. **Notify and ask:**
   ```bash
   ~/.bob/bin/bob notify plan_approval "Worktree Sync" "<plan_name> — approve merge" --plan-path "<plan_path>" 2>/dev/null || true
   ```
...
8. **Handle choice:**
   ...
   **Squash merge:**
   ...
   ~/.bob/bin/bob worktree sync --json <plan_slug> && PROJECT_ROOT=$(~/.bob/bin/bob worktree cleanup --force --json <plan_slug> | python3 -c "import sys,json; print(json.load(sys.stdin)['project_root'])") && cd "$PROJECT_ROOT"
   ```
...
2. Register: `~/.bob/bin/bob register-plan "<plan_path>" "VERIFIED" 2>/dev/null || true`
...
3. Register: `~/.bob/bin/bob register-plan "<plan_path>" "PENDING" 2>/dev/null || true`
